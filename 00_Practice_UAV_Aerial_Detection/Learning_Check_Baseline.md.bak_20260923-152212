...
权威关系：
- 当前科研唯一事项：`00_Overview/Current_Stage.md`（EI 稿）。
...
| 8 | `07_Mainline_A_Knowledge_Chain.md` | L3 | TODO | **五方法主链** |
...
**建议：** 若正并行写 EI 稿，可在 L0 串讲达标后，优先把 `03→04→06→07→08` 学完；`02` 仍须在引用 Run／复现清单前 `PASSED`。
...
知识链：冻结权重→五方法→切片映射→融合→原图评价→耗时→Stage F→C1/C2。  
通过：五方法流程图、Stage 纪律说明、F1280 vs DensK1 CI 解读、必答。
