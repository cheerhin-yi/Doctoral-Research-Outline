...
恢复后的目标是形成可复现的YOLO铁路危险检测主线和完整初稿，具体周期在A/B完成后重新制定。YOLO11负责已知危险实时检测及铁路小目标能力；开放集基线用于补充未见危险候选；轨道解析与侵界关系用于风险告警。前置论文A已经独立承担条件校准，因此Paper 1只复用其接口或做必要开放世界扩展，不重复相同主张。
