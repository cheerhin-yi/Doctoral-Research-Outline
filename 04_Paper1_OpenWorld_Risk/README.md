...
当前目标是在本轮16个弹性研究周内形成可复现的YOLO铁路危险检测主线和完整初稿。YOLO11负责已知危险实时检测及铁路小目标能力；YOLO-World/开放集基线用于补充未见危险候选；轨道解析与侵界关系用于风险告警；校准用于人工复检排序。RT-DETR只作参考对照，空间表示稳定性只作风险环节的支撑消融。
