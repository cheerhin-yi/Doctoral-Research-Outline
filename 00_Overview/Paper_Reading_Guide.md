...
每周新增论文统一执行[论文审计流程](Literature_Tracking_Workflow.md)。下方旧清单保留历史用语，新审计记录使用四级优先级：

| Reading Priority | 判断依据 | 旧用语对应 |
|---|---|---|
| Must Read | 直接影响候选创新、强基线、评价协议或研究边界，包括直接挑战主张的论文 | MUST、MUST-1/2/3、精读 |
| Recommended | 方法/实验有明确可迁移价值，但不直接决定当前主张 | SHOULD、重点 |
| Reference Only | 仅提供背景、术语或未来方向参考 | OPTIONAL、了解 |
| Irrelevant | 与现有问题、接口及证据需求无实际关系；保留排除理由，不新存正文 | EXCLUDED中的无关项 |

“暂缓”是阶段限制，不是优先级；旧记录只有再审计后才正式转换，不能机械把所有暂缓项降级。优先级、阅读状态、审计状态和发表状态相互独立，Must Read也不代表现在可以精读。
