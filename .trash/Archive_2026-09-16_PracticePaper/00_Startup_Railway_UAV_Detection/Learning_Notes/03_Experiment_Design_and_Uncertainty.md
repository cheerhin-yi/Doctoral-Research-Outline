...
1. **从论文主张得到可证伪实验**
   - 复述要求：讲清主张、假设、唯一自变量、控制变量和成功门之间的关系。
   - 验证方式：把一个方法主张改写成可失败的假设，并给出只改变一个因素的对照。
2. **公平比较与无泄漏实验单位**
   - 复述要求：说明强基线、固定训练预算和独立数据单位为何共同决定比较是否有效。
   - 验证方式：审查一个划分与对照案例，指出混杂因素或泄漏并给出修正方案。
3. **从重复结果形成有限主张**
   - 复述要求：区分单次指标、效应量、运行波动和失败原因，说明它们如何限制结论。
   - 验证方式：根据一组模拟的重复运行结果，写出允许主张、不能主张和还需补充的证据。

## 🔗 主题知识链

> 论文主张 → 可证伪假设 → 唯一变量与对照 → 数据划分 → 指标与运行 → 结果 → 保留、收缩或删除主张
