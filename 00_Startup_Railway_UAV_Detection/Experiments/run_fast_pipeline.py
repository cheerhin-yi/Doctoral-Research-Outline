"""BTD6 shared NMS replacement: actual-input equivalence and complete timing."""
import hashlib
import json
import math
import os
...
def main():
    out=BASE/'BTD6-PIPELINE-20260913-01';out.mkdir(exist_ok=False)
    ...
    try:
        import torch
        import psutil
        from fast_stable_nms import nms as fast_nms
        ...
        old_manifest=json.loads((old/'output_manifest.json').read_text())
        identity_files=['data_identity.json','summary.json']+[f'{prefix}_{i:03d}.{ext}' for i in range(500) for prefix,ext in [('image','json'),('predictions','npz')]]
        old_hashes={name:sha(old/name) for name in identity_files}
        for name,h in old_hashes.items():
            assert h==old_manifest[name]['sha256'],name
        dump(out/'prior_input_hashes.json',old_hashes)
        process=psutil.Process();resources=[];checked_nms=[];audit_nms=True
        exact_old_outputs=0;max_old_abs_diff=0.0
        def nms(pred,cap=500):
            actual=fast_nms(pred,cap)
            if audit_nms:
                expected=reference_nms(pred,cap)
                assert np.array_equal(expected,actual),'Actual-input NMS mismatch'
                checked_nms.append(dict(input_rows=len(pred),output_rows=len(actual),exact=True))
            return actual
        import cv2
        import ultralytics
        ...
        dump(out/'environment.json',dict(torch=torch.__version__,ultralytics=ultralytics.__version__,gpu=torch.cuda.get_device_name(0),
             ...
             weight_sha256=WEIGHT_SHA,script_sha256=sha(__file__),protocol_sha256=sha(Path(__file__).with_name('BTD6_Pipeline_Protocol.md'))))
        for name in ['run_fast_pipeline.py','fast_stable_nms.py','run_diag500_pair.py','diagnose_bt1.py','compare_dedup_density.py','analyze_density_residuals.py','BTD6_Pipeline_Protocol.md']:
            shutil.copy2(Path(__file__).with_name(name),out/name)
        def guard():
            mem=process.memory_info();vm=psutil.virtual_memory()
            sample=dict(elapsed_seconds=time.monotonic()-started,rss_bytes=mem.rss,private_bytes=mem.private,system_available_bytes=vm.available)
            resources.append(sample)
            if sample['elapsed_seconds']>1800 or shutil.disk_usage(out).free<5*(1<<30) or mem.rss>3*(1<<30) or mem.private>4*(1<<30) or vm.available<256*(1<<20):
                dump(out/'resource_failure.json',sample);raise RuntimeError('Resource/time guard')
        def read_image(name):
        ...
        dump(out/'actual_input_nms_checks.json',checked_nms)
        audit_nms=False
        torch.cuda.reset_peak_memory_stats()
        rgb,_,_=read_image(cal[0])
        for m in ['density','dedup','F1280']:
            for _ in range(20):pipeline(rgb,m)
        print('Identity and cal48 actual-input NMS equivalence PASS; warmup completed',flush=True)
        cal_times=[]
        ...
        for i,name in enumerate(diag):
            ...
            old_record=json.loads((old/f'image_{i:03d}.json').read_text())
            assert old_record['image']==name and old_record['image_sha256']==image_sha
            with np.load(old/f'predictions_{i:03d}.npz') as saved:
                old_pred={m:saved[m].copy() for m in ['density','dedup']}
            for rep in range(3):
                methods=['density','dedup'] if (i+rep)%2==0 else ['dedup','density']
                for m in methods:
                    pred,sel,t,full=pipeline(rgb,m)
                    expected_pred=old_pred[m]
                    stable=sel==old_record['selected'][m] and pred.shape==expected_pred.shape and np.allclose(pred,expected_pred,atol=1e-5,rtol=0)
                    if not stable:
                        np.savez_compressed(out/'mismatch.npz',actual=pred,expected=expected_pred)
                        dump(out/'mismatch.json',dict(image=name,repeat=rep,method=m,selected=sel,expected_selected=old_record['selected'][m]))
                        raise RuntimeError('Output changed versus BTD4')
                    exact_old_outputs+=int(np.array_equal(pred,expected_pred))
                    if pred.size:max_old_abs_diff=max(max_old_abs_diff,float(np.max(np.abs(pred-expected_pred))))
            ...
            for field in ['annotation_sha256','width','height','small_gt','valid_gt','metrics','selected','gained_small','lost_small']:
                assert record[field]==old_record[field],(name,field)
            ...
            if (i+1)%25==0:
                ...
                write_csv(out/'resource_samples.csv',resources)
                if sum(p.stat().st_size for p in out.rglob('*') if p.is_file())>1<<30:raise RuntimeError('Output size guard')
        ...
        assert summary['repeat_output_mismatches']==0
        for field in ['metrics','small_gt','valid_gt','gained_small','lost_small','classes']:
            assert json.loads(json.dumps(summary[field]))==old_summary[field],field
        assert all(sha(old/name)==h for name,h in old_hashes.items())
        write_csv(out/'resource_samples.csv',resources)
        summary.update(actual_input_nms_checks=len(checked_nms),old_output_comparisons=3000,exact_old_outputs=exact_old_outputs,max_old_absolute_difference=max_old_abs_diff,
             resource_samples=len(resources),max_sampled_rss_bytes=max(r['rss_bytes'] for r in resources),max_sampled_private_bytes=max(r['private_bytes'] for r in resources),
             min_sampled_available_bytes=min(r['system_available_bytes'] for r in resources),gpu_peak_allocated_bytes=torch.cuda.max_memory_allocated(),gpu_peak_reserved_bytes=torch.cuda.max_memory_reserved(),
             prior_inputs_unchanged=True,training_calls=0)
